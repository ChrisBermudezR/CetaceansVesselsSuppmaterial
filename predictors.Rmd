...
The selection of environmental predictors was carried out after discarding any correlations  

To avoid over-fitting models, variable selection was performed using a heuristic approach [@Cobos2019]. For this, a pairwise Spearman correlation matrix was first constructed to detect statistical correlation between the environmental predictors used in the habitat suitability models. Because two of the selected cetacean species and one subspecies only occur in one of the two ocean basins within the Colombian EEZ, it was necessary to perform this analysis on two datasets, each with layers representing only the Caribbean and Pacific, respectively. This analysis was performed using the "*corrplot*" package of the R program (v.0.84; @Wei2017). A posteriori and using all previously uncorrelated environmental predictors, several runs for predictor testing were performed using the Maximum Entropy algorithm [MaxEnt; @Elith2011; @Gomes2018; @Phillips2006] with the default settings (*e.g.*, randomly generated background, no cross-validation for replicates) to identify predictors contributing most to each species/subspecies model based on significance value greater than 0.05. MaxEnt allows discriminating between predictors for each species, as its analysis is based on background data to build habitat suitability models [@Hirzel2002], finding the environmental conditions with the maximum entropy at recorded species sighting points using a log-linear model [@Merow2013; @Merow2014].

Finally, environmental predictors with a significance value lower than 0.05 were excluded, so the remaining predictors were selected to perform habitat suitability modeling based on previous studies, the ecology of each cetacean species/subspecies and their natural history (Table 1) [@Redfern2017].

In addition, and based on the hypothesis that marine features In addition, and based on the hypothesis that marine features and dynamic oceanographic conditions influence the distribution of marine cetaceans, we selected environmental predictors that remained within the boundaries of the meso- and macro-scales of sense of dolphins and baleen whales (10 km to 1,000 km distance) [@Torres2017].
